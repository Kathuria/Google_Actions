  ...
  return axios
    ...
    .then(coordinates => {
      ...
      var place_search =
        ...
        "&inputtype=textquery&fields=photos,formatted_address,name,opening_hours,rating&locationbias=circle:" +
        user_proximity +
        "@" +
        lat +
        "," +
        ...
        "&key=" +
        api_key;
      return axios.get(place_search).then(response => {
        var photo_reference =
        ...
        conv.close(
          `Okay, the restaurant name is ` +
            name +
            ` and the address is ` +
            address +
            `. The following photo uploaded from a Google Places user might whet your appetite!`
        );
      });
    });
});
